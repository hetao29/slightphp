...
class SError extends Exception{
	...
	function getErrorText($backtrace,$e=null){
		...
		if(!empty($backtrace[$i]['args']) &&!empty($backtrace[$i]['args'][0]) &&!empty($backtrace[$i]['args'][1])){
			...
			$text.=($index++)."\t".$backtrace[$i]['args'][2]."(".$backtrace[$i]['args'][3].")\t".SError::$error_type[$errorCode].':'.(!empty($backtrace[$i]['args'])?$backtrace[$i]['args'][1]:"")."\r\n";
		}elseif($e){
	...
	function getErrorHtml($backtrace,$e=null){
		...
		if(!empty($backtrace[$i]['args']) &&!empty($backtrace[$i]['args'][0]) &&!empty($backtrace[$i]['args'][1])){
			//error
			$errorCode = $backtrace[$i]['args'][0];
			$html.='<tr style="background-color: #cccccc; color: #000000;"><td>'.($index++).'</td><td>'.$backtrace[$i]['args'][2].'</td><td>'.$backtrace[$i]['args'][3].'</td><td></td><td style="font-weight:bold">'.SError::$error_type[$errorCode].':'.(!empty($backtrace[$i]['args'])?$backtrace[$i]['args'][1]:"").'</td></tr>';
		}elseif($e){
